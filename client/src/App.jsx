...
/* ============================ ASSISTANT IA (OpenAI via API Darna) ============================ */
...
async function askClaude(system, messages) {
  ...
  const restaurantIds = (data.restaurants || []).map((restaurant) => RESTOS.find((r) => r.name === restaurant.name)?.id).filter(Boolean);
  return { out: { reply: data.answer, interpretation: data.answer, restaurantIds }, text: data.answer };
}
    ...
    <div style={{ minHeight: "100vh", width: "100%", background: dark ? "#05070B" : "#DFD6C4",
      ...
      <style>{`
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .darna-shell { max-width: 1180px; }
        .map-canvas .leaflet-control-attribution { font-size: 9px; }
        @media (min-width: 768px) {
          .darna-shell { width: min(100%, 1040px) !important; height: min(100dvh - 48px, 900px) !important; border-radius: 32px !important; }
          .device-status { display: none !important; }
        }
      `}</style>

      <div className="darna-shell" style={{ width: "100%", height: "100dvh", background: c.bg, borderRadius: 0, overflow: "hidden",
        position: "relative", boxShadow: "0 40px 80px -20px rgba(0,0,0,.5),0 0 0 11px #0b0d12,0 0 0 13px #23262e",
        display: "flex", flexDirection: "column" }}>

        <div className="device-status" style={{ height: 44, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between",
...
/* ============================ CARTE INTERACTIVE ============================ */
function MapBounds({ restaurants, userPosition }) { const map = useMap(); useEffect(() => { const points=[...restaurants.map(r=>[r.lat,r.lng]),[userPosition.lat,userPosition.lng]]; map.fitBounds(points,{padding:[32,32],maxZoom:14}); },[map,restaurants,userPosition]); return null; }
function MapScreen({ c, city, openResto, display }) {
  const list = RESTOS.filter((r) => r.city === city); const [sel,setSel]=useState(list[0]||RESTOS[0]); const [userPosition,setUserPosition]=useState(USER_POS);
  useEffect(()=>{setSel(list[0]||RESTOS[0])},[city]); useEffect(()=>{if(!navigator.geolocation)return; navigator.geolocation.getCurrentPosition(({coords})=>setUserPosition({lat:coords.latitude,lng:coords.longitude,label:'Votre position'}),()=>{}, {enableHighAccuracy:false,timeout:7000,maximumAge:300000});},[]);
  const dt=distTime(sel); const directions=`https://www.google.com/maps/dir/?api=1&destination=${sel.lat},${sel.lng}`;
  return <div style={{height:'100%',position:'relative'}}>
    <MapContainer className="map-canvas" center={[userPosition.lat,userPosition.lng]} zoom={13} scrollWheelZoom style={{height:'100%',width:'100%',background:c.surface}}>
      <TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
      <MapBounds restaurants={list} userPosition={userPosition}/>
      <CircleMarker center={[userPosition.lat,userPosition.lng]} radius={9} pathOptions={{color:'#fff',weight:3,fillColor:c.primary,fillOpacity:1}}><Popup>{userPosition.label}</Popup></CircleMarker>
      {list.map(r=><CircleMarker key={r.id} center={[r.lat,r.lng]} radius={sel.id===r.id?15:12} pathOptions={{color:'#fff',weight:3,fillColor:sel.id===r.id?c.gold:c.primary,fillOpacity:1}} eventHandlers={{click:()=>setSel(r)}}><Popup><strong>{r.name}</strong><br/>{r.rating} ★ · {r.cuisine}</Popup></CircleMarker>)}
    </MapContainer>
    <div style={{position:'absolute',top:14,left:20,right:20,zIndex:500,background:c.surface,borderRadius:16,padding:'12px 16px',display:'flex',alignItems:'center',gap:10,boxShadow:'0 8px 24px rgba(0,0,0,.14)'}}><Search size={18} color={c.inkSoft}/><span style={{color:c.inkSoft,fontSize:14,flex:1}}>Carte de {city}</span><MapPin size={18} color={c.primary}/></div>
    <div className="card" style={{position:'absolute',bottom:88,left:20,right:20,zIndex:500,background:c.surface,borderRadius:20,padding:14,boxShadow:'0 14px 34px rgba(0,0,0,.2)',border:`1px solid ${c.line}`}}>
      <div onClick={()=>openResto(sel.id)} className="press" style={{display:'flex',gap:13,alignItems:'center',cursor:'pointer'}}><DishArt r={sel} rounded={14} style={{width:56,height:56}}/><div style={{flex:1,minWidth:0}}><div style={{display:'flex',gap:6,alignItems:'center'}}><span style={{fontWeight:700,fontSize:15.5,color:c.ink}}>{sel.name}</span>{sel.verified&&<CheckCircle2 size={14} color={c.gold}/>}</div><div style={{fontSize:12.5,color:c.inkSoft,margin:'3px 0'}}>{sel.cuisine} · {priceStr(sel.price)} · {sel.city}</div><div style={{display:'flex',gap:12,fontSize:12.5,fontWeight:600}}><span style={{color:c.primary,display:'flex',gap:4,alignItems:'center'}}><Navigation size={13}/>{dt.kmStr}</span><span style={{color:c.gold,display:'flex',gap:4,alignItems:'center'}}><Clock size={13}/>{dt.tStr}</span></div></div></div>
      <div style={{display:'flex',gap:10,marginTop:12}}><a className="press" href={directions} target="_blank" rel="noreferrer" style={{flex:1,background:c.primary,color:'#fff',border:'none',borderRadius:13,padding:'11px 0',fontSize:14,fontWeight:700,cursor:'pointer',display:'flex',gap:7,alignItems:'center',justifyContent:'center'}}><Navigation size={16}/>Itinéraire</a><a href={`tel:${sel.phone}`} className="press" style={{background:c.goldSoft,border:`1.5px solid ${c.gold}`,color:c.goldInk,borderRadius:13,padding:'0 16px',display:'flex',alignItems:'center',cursor:'pointer'}}><Phone size={17}/></a></div>
    </div>
  </div>;
}
...
/* ---------- Écran 4 : Inscription ---------- */
function SignUp({ t, dir, onBack, onDone, lang }) {
  ...
  const allValid = v.first && v.last && v.email && v.phone && v.password;
  ...
  const submit = async () => {
    ...
    try { const { data } = await api.post("/auth/register", { firstName: f.first, lastName: f.last, email: f.email, phone: dial.d + " " + f.phone, password: f.password, language: lang }); localStorage.setItem("darna_access_token", data.accessToken); onDone({ ...f, phone: dial.d + " " + f.phone }); }
    catch (error) { setSubmitError(error.response?.data?.message || "Unable to create your account. Please try again."); setLoading(false); }
  };
  ...
  return (
    <div dir={dir} style={{ position: "absolute", inset: 0, background: "#fff", display: "flex", flexDirection: "column" }}>
      ...
      <div className="scr" style={{ flex: 1, overflowY: "auto", padding: "16px 26px 0" }}>
        ...
        <Field icon={<Check size={19} />} label="Password" value={f.password} onChange={set("password")} onBlur={blur("password")}
          valid={v.password} error="Use at least 8 characters" touched={touched.password} type="password" dir="ltr" />
        {submitError && <p role="alert" style={{ color: "#B42318", fontSize: 13, lineHeight: 1.4, margin: "4px 0" }}>{submitError}</p>}
      </div>
    ...
    <div style={{ minHeight: "100vh", width: "100%", background: "#DFD6C4", display: "flex", alignItems: "center",
      ...
      <style>{`
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .onboarding-shell { width: 100%; height: 100dvh; border-radius: 0; box-shadow: none; }
        @media (min-width: 768px) { .onboarding-shell { width: min(100%, 520px); height: min(100dvh - 48px, 800px); border-radius: 44px; box-shadow: 0 40px 80px -20px rgba(0,0,0,.5),0 0 0 11px #0b0d12,0 0 0 13px #23262e; } }
        @keyframes spin { to { transform: rotate(360deg); } }
      ...
      {step === "app" ? (
        ...
        <div className="onboarding-shell" style={{ background: "#fff", overflow: "hidden", position: "relative" }}>
          <div key={step} style={{ position: "absolute", inset: 0, animation: "screenIn .4s ease both" }}>
            ...
            {step === "signup" && <SignUp t={t} dir={t.dir} lang={lang} onBack={() => setStep("lang")} onDone={(u) => { setUser(u); setStep("success"); }} />}
